...
## 3.2UML用例建模
需求分析的结果是想获得用例模型，当然还可能有其他种类的产品，如用户界面原型。用例模型是一种包括用例、参与者以及它们之间关系的系统模型，用例模型可以使软件开发人员和客户之间在需求（即系统必须满足的条件和能力）方面达成共识。用例模型充当客户和开发人员之间勾通的桥梁，并作为分析、设计和测试的基本输入。 1、确定参与者（actor）<br>
...
参与者使用系统的每一种方法都可以表示为一个用例。用例是系统能够向其参与者提供增值的功能“块”。更严格地说，用例确定了一个与系统参与者进行交互、并可由系统执行动作的序列。一个用例是一种规格说明。在本系统中，涉及到的用例在下表系统用例清单中列出。
系统用例清单
<table>
 <tr>
  <td>用例名称</td>
